    ...
    useEffect(() => {
        const fetchHistory = async () => {
            if (authState && authState.isAuthenticated) {
                const url = `${process.env.REACT_APP_API}/histories/search/findByUserEmail/?userEmail=${authState.accessToken?.claims.sub}&page=${currentPage - 1}&size=5`;
                const requestOption = {
                    method: 'GET',
                    headers: {
                ...
                const responseData = historyJson._embedded.histories;
        
                setHistories(responseData);
                setTotalPages(historyJson.totalPages);
