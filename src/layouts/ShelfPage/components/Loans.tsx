...
import { ShelfCurrentLoans } from "../../../Model/ShelfCurrentLoan";
    ...
    async function returnBook(bookId:number) {
        if (authState && authState.isAuthenticated) {
            const returnBook = async () => {
                ...
                const currentLoanResponse = await fetch(url, requestOption);
                if (!currentLoanResponse.ok) {
                  throw new Error("Something wet Wrong");
                }
                setCheckout(!checkout);
            }
            returnBook().catch((error: any) => {
  ...
  useEffect(() => {
    const fetchCurrentLoan = async () => {
      if (authState && authState.isAuthenticated) {
        ...
        setShelfCurrentLoans(currentLoanJson);
      }
      setIsLoadingUserLoans(false);
    };
    fetchCurrentLoan().catch((error: any) => {
      setHttpError(error.message);
      setIsLoadingUserLoans(false);
    });
    window.scrollTo(0, 0);
